...
@dataclass
class DXBNMFARDConfig():
    """Docstring for DXBNMFARDConfig. """
    eps: float = 2.223e-16
    matrix_seed: int = 1
...
class CUSTOM(Model):
    """Docstring for DXBNMFARD. """
    config = DXBNMFARDConfig()
    ...
    def upd_z(self) -> "numpy.matrix":
        ...
        """
        return np.multiply(self._vars["Z"] / ((1 - self.alpha) * self._vars["H_1"] * np.ones(self._vars["W"].shape) + \
    ...
    def upd_g(self) -> "numpy.matrix":
        ...
        return (np.multiply(self._vars["G"], (self._vars["F"] / (self._vars["G"] * \
                self._vars["H"] + self.config.eps)) * self._vars["H"].T)) / \
                (np.ones(self._vars["F"].shape) * self._vars["H"].T + self.config.eps)
