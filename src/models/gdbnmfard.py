...
@dataclass
class GDBNMFARDConfig():
    """Docstring for GDBNMFARDConfig. """
    eps: float = 2.223e-16
    matrix_seed: int = 1
...
class GDBNMFARD(Model):
    """Docstring for GDBNMFARD. """
    config = GDBNMFARDConfig()
    def __init__(self, data: "Data", time: int, config: "Config") -> None:
        ...
        mat_w_t1, mat_h_t1, mat_g_t1 = data.get_past_data(time, num_k, config.attrFolder, config.h)
        if time != 0:
    ...
    def upd_h(self) -> "numpy.matrix":
        ...
        return (np.multiply(self.alpha * self._vars["H"], \
                ...
                self.config.eps))) + (1 - self.alpha) * self._vars["H_1"]) / \
    ...
    def comm(self, method: str = "W", threshold: float = 0.5, overlapping: bool = False) -> "numpy.ndarray":
        M = self._vars["W"].copy() if method == "W" else self._vars["H"].T
        if overlapping:
            M /= M.sum(axis=1)
            values = np.argwhere((M == np.amax(M, axis=1)) | (M > threshold))
            mapping = {idx: val for idx, val in enumerate(sorted(self._data.get_graph(self._time).nodes))}#enumerate(self._data.get_graph(self._time).nodes)}##Renny
    ...
    def update_data(self) -> None:
        ...
        """
        self._data.set_past_dataD(self._time, self._vars["W"], self._vars["H"])
