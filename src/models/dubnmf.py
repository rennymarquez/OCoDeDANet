...
class CUSTOM(Model):
    ...
    def __init__(self, data: "Data", time: int, config: "Config") -> None:
        ...
        self._vars = {
                "V": adj_matrix,
                "F": attr_matrix,
                "W_1": np.asmatrix(np.random.rand(adj_matrix.shape[0], num_k)),
                "H_1": np.asmatrix(np.random.rand(num_k, adj_matrix.shape[1])),
                "G_1": np.asmatrix(np.random.rand(attr_matrix.shape[0], num_k)),
