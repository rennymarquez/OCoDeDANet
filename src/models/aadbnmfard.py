...
class AADBNMFARD(Model):
    ...
    def upd_h(self) -> "numpy.matrix":
        ...
        return np.multiply(self._vars["H"] / (self.alpha * self._vars["W"].T * np.ones(self._vars["V"].shape) \
                ...
                (self._vars["G"] * self._vars["H"] + self.config.eps)) + \
                (1 - self.alpha) * (self._vars["Z"].T * self._vars["H_1"]) / \
                (self._vars["H"] + self.config.eps))
    ...
    def upd_z(self) -> "numpy.matrix":
        ...
        """
        return np.multiply(self._vars["Z"] / ((1/2) * (self._vars["H_1"] / \
            (np.ones(self._vars["Z"].shape) * self._vars["Z"].T * self._vars["H_1"] \
            + self.config.eps)) * np.ones(self._vars["H"].T.shape) + self._vars["Z"] \
            * self._vars["B"] + self.config.eps), ((1 - self.alpha) / self.alpha) * self._vars["H_1"] \
            * (np.maximum(self.config.eps, np.log(self._vars["H"] / (self._vars["Z"].T * \
            self._vars["H_1"] + self.config.eps) + self.config.eps))).T)
    ...
    def comm(self, method: str = "W", threshold: float = 0.5, overlapping: bool = False) -> "numpy.ndarray":
        ...
        if overlapping:
            M /= M.sum(axis=1)
            return np.argwhere((M == np.amax(M, axis=1)) | (M > threshold))
