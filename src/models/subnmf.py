...
class SUBNMF(Model):
    ...
    def update(self) -> None:
        ...
        self._vars["B"] = (self.N + self._vars["F"].shape[0] + self.config.a - 1) * \
                np.identity(self.K) / np.array( \
                (np.multiply(self._vars["W"], self._vars["W"]).sum(axis=0) + \
