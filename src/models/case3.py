...
class CUSTOM(Model):
    ...
    def upd_h(self) -> "numpy.matrix":
        ...
        return (np.multiply(self.alpha * self._vars["H"], \
                ...
                self.config.eps))) + (1 - self.alpha) * self._vars["Z"].T * self._vars["H_1"]) / \
    ...
    def upd_z(self) -> "numpy.matrix":
        ...
        """
        return np.multiply(self._vars["Z"] / ((1 - self.alpha) * self._vars["H_1"] * \
                np.ones(self._vars["W"].shape) + \
    ...
    def update_data(self) -> None:
        ...
        """
        self._data.set_past_data(self._time, self._vars["W"], self._vars["H"], None)
